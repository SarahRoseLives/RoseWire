...
class SshChatService {
        ...
        if (response.statusCode == 200) {
            ...
            if (serverVersion != null) {
                if (serverVersion != clientVersion) {
                    _versionStatusController.add("Warning: Server is running version $serverVersion, but client expects $clientVersion.");
                } else {
  ...
  Future<void> connect({
      ...
      try {
          ...
          _healthCheckTimer?.cancel();
          _healthCheckTimer = Timer.periodic(const Duration(seconds: 15), (_) => _healthCheck());
